...
- バックグラウンド定期生成には `WorkManager` を使う。
- 厳密な時刻実行は保証しない。
...
生成画像はアプリ内に保存する。
必要に応じて、適用用にリサイズまたはクロップした画像も保存する。
...
バックグラウンド定期生成では `WorkManager` を使う。
...
バックグラウンド定期生成は、`WorkManager`、アプリ起動時、フォアグラウンド復帰時に実行可否を確認する。
...
生成処理が長時間化する場合、またはOSによりバックグラウンド処理が中断されやすい場合は、Foreground Workerとして実行する。
...
```text
UI:
  HomeViewModel -> GenerationCoordinator.runManual()

Worker:
  BackgroundGenerationWorker -> GenerationCoordinator.runScheduled()
```
...
data class HistoryItem(
    ...
    val applyTarget: WallpaperApplyTarget?,
)
```
...
定期生成は `RunsPerDay` のスロット制で管理し、`WorkManager` で実行を試みる。
