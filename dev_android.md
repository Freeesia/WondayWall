...
WondayWall Android版は、ユーザーの予定・ニュース・ユーザープロンプトをもとに、Android端末向けの壁紙画像を生成し、ホーム画面へ適用するアプリである。
...
- カレンダー予定は、Android OSのCalendar Provider / `CalendarContract` から取得する。
- 初期版ではGoogle Calendar APIを直接呼び出さない。
- ホーム画面壁紙は常に適用対象とする。
- ロック画面はWindows版の `UpdateLockScreen` と同じく、追加で更新するかどうかを bool で管理する。
- ロック画面だけを更新するモードは用意しない。
- 壁紙適用前の画像クロップは行わない。
- 厳密な時刻実行は保証しない。
...
| 文脈作成 | `ContextService` | OSカレンダー、RSS、ユーザープロンプトから生成文脈を作る |
| 画像生成 | `GoogleAiService` | Google AI呼び出しと生成画像保存を担当する |
| 壁紙処理 | `WallpaperService` | 壁紙適用、写真保存、共有を担当する |
| 生成統括 | `GenerationCoordinator` | UIとWorkManager実行の両方から呼ばれる中心クラス |
| 履歴 | `HistoryService` | 成功、失敗、スキップ履歴を保存する |
| 定期実行 | `TaskSchedulerService` | Androidでは内部でWorkManagerを使う |

Android固有の主要サービス名は増やさない。
WorkManagerから起動される処理の入口、通知処理、OSカレンダー取得処理はAndroidアプリ内部の実装詳細として扱い、この仕様では独立した主要サービスクラス名として定義しない。

## 提供機能

- Google AIによるAndroid向け壁紙画像の生成
- OSカレンダーの予定を使った生成コンテキスト作成
...
- ロック画面だけの壁紙更新
- 画像のクロップ
- Android版初期実装でのGoogle Calendar API直接利用
- Google OAuthによるカレンダー予定取得
- jobIdによる生成結果の後追い取得
- 自前中間サーバー
- 複数端末同期
- 複数ランチャー固有仕様への完全対応
...
- カレンダー取得: Calendar Provider / `CalendarContract`
- 壁紙適用: WallpaperManager
- 画像読み込み/表示: Coil
- 画像読み込み: Bitmap / ImageDecoder
...
Android版では、予定情報の取得にAndroid OSのCalendar Provider / `CalendarContract` を使用する。
初期版ではGoogle Calendar APIを直接呼び出さない。

### 方針

- `READ_CALENDAR` 権限をユーザーに要求する。
- ユーザーが許可した場合のみ、端末に同期されているカレンダー予定を取得する。
- Googleカレンダー、Exchange、その他カレンダーアカウントは、端末に同期されていれば取得対象にできる。
- Google Calendar API用のOAuthは初期版では使用しない。
- カレンダー予定の書き込みは行わない。
- `WRITE_CALENDAR` は要求しない。
- 取得した予定は画像生成用のコンテキストにのみ使用する。
- 取得した予定本文を外部共有しない。
- 生成履歴には、生成に使った予定の要約のみ保存する。

### 取得できる範囲

取得できる予定は、端末に同期済みのカレンダー予定に限る。
Google Calendar上に存在していても、端末側でアカウントが追加されていない、同期が無効、対象カレンダーが未同期の場合は取得できない。

### ContextServiceでの扱い

`ContextService` は、OSカレンダー予定、RSS/ニュース、ユーザープロンプトを統合して `PromptContext` を作成する。

```text
ContextService
  -> CalendarContract から端末カレンダー予定を取得
  -> RSS/ニュースを取得
  -> ユーザープロンプトを取得
  -> PromptContext を作成
```

OSカレンダー取得用に主要サービスクラス名を追加しない。
必要な場合でも、Android実装内部のprivate/helperクラスとして扱う。

### カレンダー選択

ユーザーが取得対象カレンダーを選べるようにする。
設定値はWindows版の `targetCalendarIds` と同名にする。
Android版では、`targetCalendarIds` にCalendar Provider上のカレンダーIDを保存する。

### 権限拒否時

ユーザーが `READ_CALENDAR` を拒否した場合でも、アプリ自体は利用可能にする。
その場合は、RSS/ニュースとユーザープロンプトのみで生成コンテキストを作成する。
...
生成画像は写真領域の `Pictures/WondayWall` に保存し、この保存先 URI を正本として履歴に保持する。
画像生成サービスが作成するアプリ内ファイルは、写真領域へ保存するまでの一時ファイルとして扱う。
壁紙適用時は、基本的に写真領域に保存した画像をそのまま `WallpaperManager` に渡す。
画像のクロップは行わない。

```text
Pictures/WondayWall/
  Google AIから受け取った生成画像の正本

cache/wallpaper-staging/
  写真領域へ保存するまでの一時画像
...
初期版では、現在端末の画面サイズに厳密に合わせるための加工は行わない。
スクロール壁紙向けの横幅拡張も将来対応とする。
...
適用先はWindows版の `UpdateLockScreen` と同じ考え方にする。
...
- `updateLockScreen = false`: ホーム画面のみ更新する。
- `updateLockScreen = true`: ホーム画面に加えてロック画面も更新する。

ロック画面だけを更新する設定は用意しない。

### 方針

- ホーム画面への適用を基本機能とする。
- `updateLockScreen` が有効な場合のみ、ホーム画面に加えてロック画面へも適用する。
...
- 画像ファイルの読み込み
- ホーム画面壁紙への適用
- `updateLockScreen` が有効な場合のロック画面壁紙への追加適用
- 写真/ギャラリー保存
- 共有シート表示
- 適用失敗時のエラー返却

`WallpaperService` は、初期版では壁紙用の画像加工を行わない。
...
5. 画像をアプリ内に保存する
6. WallpaperServiceが壁紙に適用する
7. HistoryServiceが履歴を保存する
8. 画面を更新する
```
...
生成処理が長時間化する場合、またはOSによりバックグラウンド処理が中断されやすい場合は、WorkManagerのForeground実行を使う。

Foreground実行を使う場合は、生成中であることを示す通知を表示する。
...
Android版では、OSカレンダーの予定を `CalendarContract` から取得する。
Google Calendar APIは初期版では直接使用しない。
...
UIとWorkManager実行の両方からこのクラスを呼ぶ。
処理本体を画面側やWorkManager側に分散させない。
...
- 画像の存在確認
- ホーム画面壁紙への適用
- `UpdateLockScreen` 有効時のロック画面壁紙への追加適用
- 写真/ギャラリー保存
- 共有シート表示
- 適用失敗時のエラー返却
...
`READ_CALENDAR` を要求する。
端末に同期されているカレンダー予定を取得するために使う。

初期版では予定の書き込みを行わないため、`WRITE_CALENDAR` は要求しない。
Google Calendar APIを直接使用しないため、カレンダー予定取得用のGoogle OAuthも初期版では使用しない。
...
生成画像を写真領域の `Pictures/WondayWall` に保存するために使う。
生成成功には写真領域への保存成功を必須とし、Android 10以上では `MediaStore`、Android 9以下では `WRITE_EXTERNAL_STORAGE` 権限付きで公開 Pictures 配下へ保存する。
...
`googleAiApiKey`, `targetCalendarIds`, `rssSources`, `userPrompt`, `schedule`, `skipGenerationWhenNoChanges`, `updateLockScreen` はWindows版と同名にする。
Android版の `targetCalendarIds` は、Calendar Provider上のカレンダーIDを保存する。
...
`CalendarEventItem` は、Android版ではCalendar Providerから取得した予定を格納する。
...
```text
- カレンダー権限がない場合は、予定なしとして生成を継続する
- 端末に同期済み予定がない場合は、予定なしとして生成を継続する
- 取得処理でエラーが発生した場合は、RSS/ニュースとユーザープロンプトのみで生成する
- 必要に応じて画面上でカレンダー取得状態を表示する
```
...
手動生成とWorkManagerによる生成が同時に走らないようにする。
...
- `READ_CALENDAR` 権限を要求できる。
- 端末に同期されているカレンダー一覧を取得できる。
- 端末に同期されているカレンダー予定を取得できる。
- Google Calendar APIを直接使わずに予定取得できる。
- カレンダー権限が拒否されても、予定なしで生成を継続できる。
- RSS/ニュースを取得できる。
- ユーザープロンプトを保存できる。
- 手動生成で縦長壁紙画像を生成できる。
- 生成画像をアプリ内に保存できる。
- ホーム画面壁紙に適用できる。
- `updateLockScreen` が有効な場合、ロック画面壁紙にも適用を試みる。
- ロック画面適用に失敗した場合は履歴と画面上で確認できる。
- ロック画面だけを更新するモードが存在しない。
...
WondayWall Android版は、ユーザーの予定・ニュース・ユーザープロンプトをもとにAndroid端末向けの縦長壁紙を生成し、ホーム画面へ自動適用するアプリとする。

Windows版の `GenerationCoordinator` 中心設計と、iOS版のモバイル向けバックグラウンド生成仕様を踏襲しつつ、Androidでは `WallpaperManager` による直接適用を提供する。

主要クラス名はWindows版に揃える。
カレンダー予定はAndroid OSのCalendar Provider / `CalendarContract` から取得し、Google Calendar APIは初期版では直接使用しない。
ロック画面はWindows版の `UpdateLockScreen` と同じく、ホーム画面に加えて更新するかどうかを bool で管理する。
